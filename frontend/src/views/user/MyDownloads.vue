...
<template>
  <div class="page-container">
    <div class="page-title"><LineIcon name="download" :size="19" /> {{ $t('我的下载') }}</div>

    <el-empty v-if="!loading && items.length === 0" :description="$t('还没有下载记录')" />
    ...
    <el-card v-else class="table-card">
      <el-table :data="items" v-loading="loading">
        ...
        <el-table-column :label="$t('类型')" width="100" align="center">
          <template #default="{ row }">
            <el-tag size="small" :type="row.type === 'resource' ? 'success' : 'warning'" effect="plain">
              {{ row.type === 'resource' ? $t('学习资源') : $t('3D 模型') }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column :label="$t('下载时间')" width="160">
          <template #default="{ row }">{{ formatDate(row.downloadTime) }}</template>
        </el-table-column>
        <el-table-column :label="$t('操作')" width="120" align="center">
          <template #default="{ row }">
            <el-button size="small" type="primary" plain @click="reDownload(row)">{{ $t('再次下载') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
