...
function statusInfo(m) {
  if (m.isApproved) return { text: '已通过', type: 'success' }
  if (m.rejectionReason) return { text: '已驳回', type: 'danger' }
...
async function togglePublic(m) {
  ...
  ElMessage.success(m.isPublic ? '已下架' : '已上架')
  load()
}

async function remove(m) {
  try {
    await ElMessageBox.confirm(`确定删除模型「${m.name}」吗？删除后不可恢复`, '删除模型', { type: 'warning' })
  } catch {
    return // 用户取消
  }
  try {
...
<template>
  <div class="page-container">
    <div class="page-title"><LineIcon name="cube" :size="19" /> 模型管理</div>
    ...
    <el-table v-loading="loading" :data="rows" size="small">
      ...
      <el-table-column label="创作者" width="120" show-overflow-tooltip>
        <template #default="{ row }">{{ row.creator || '未知' }}</template>
      </el-table-column>
